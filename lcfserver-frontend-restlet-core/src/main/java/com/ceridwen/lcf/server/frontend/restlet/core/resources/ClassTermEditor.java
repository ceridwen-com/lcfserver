...
public class ClassTermEditor extends ServerResource {

	ResourceHandler<ClassTerm> handler;
	
	public ClassTermEditor() {
		handler = new ResourceHandler<>(this, EntityTypes.Type.ClassTerm);
	}

	@Get(ResourceHandler.CONSUME_PRODUCES_TYPES)
	public ClassTerm Retrieve() {
		return handler.Retrieve();
	}

	@Put(ResourceHandler.CONSUME_PRODUCES_TYPES)
	public ClassTerm Modify(ClassTerm data) {
